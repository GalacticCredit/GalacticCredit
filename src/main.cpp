...
#if defined(NDEBUG)
# error "GalacticCredit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9b56db8d92cf4d90a0c9d8ef970810fbeb6e73d0ab97de6acb02ea35b4401ae0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GalacticCredit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GalacticCredit Signed Message:\n";
...
{
    // GalacticCredit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // GalacticCredit: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // GalacticCredit: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // GalacticCredit: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GalacticCredit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GalacticCredit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b56db8d92cf4d90a0c9d8ef970810fbeb6e73d0ab97de6acb02ea35b4401ae0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Las Vegas Massacure 10-21-17";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("091284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508587363;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1833939;
        ...
        assert(block.hashMerkleRoot == uint256("0xf73385400d7edf27aad8d9506f700edb790669498b9bd67c6747100aef550a6f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GalacticCredit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GalacticCreditMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GalacticCreditMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GalacticCreditMiner(CWallet *pwallet)
{
    printf("GalacticCreditMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("galacticcredit-miner");
    ...
    try { loop {
        ...
        printf("Running GalacticCreditMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GalacticCreditMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GalacticCreditMiner, pwallet));
}
